...
SIG=$FP3_ROOT/adsp-snapHWL4-signed.mbn
...
echo "== [2] deploy patched HWL4 =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapHWL4-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapHWL4-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
sleep 12
echo "== [4] read SMEM (HWL4 stash) =="
$SCP smem_snapHWL4_read.py "$DEV:/tmp/smem_snapHWL4_read.py" >/dev/null 2>&1
echo "---- HWL4 READOUT ----"
r "python3 /tmp/smem_snapHWL4_read.py"
echo "----------------------"
