...
SIG=$FP3_ROOT/adsp-snapCKB7b-signed.mbn
...
echo "== [2] deploy patched CKB7b =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapCKB7b-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapCKB7b-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
...
sleep 12
echo "== [4] read SMEM (CKB7b stash) =="
$SCP smem_snapCKB7b_read.py "$DEV:/tmp/smem_snapCKB7b_read.py" >/dev/null 2>&1
echo "---- CKB7b READOUT ----"
r "python3 /tmp/smem_snapCKB7b_read.py"
echo "----------------------"
