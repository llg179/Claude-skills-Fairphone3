...
SIG=$FP3_ROOT/adsp-snapA-signed.mbn
...
echo "== [2] deploy patched (SNPA) =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapA-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapA-signed.mbn $FWDIR/adsp.mbn; echo -n patched=; md5sum $FWDIR/adsp.mbn"
...
echo "== [4] read SMEM (SNPA stash) =="
$SCP smem_snapA_read.py "$DEV:/tmp/smem_snapA_read.py" >/dev/null 2>&1
echo "---- SNPA READOUT ----"
r "python3 /tmp/smem_snapA_read.py"
echo "----------------------"
