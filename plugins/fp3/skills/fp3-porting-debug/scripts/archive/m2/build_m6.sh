...
#   (b) the m6trace.s cave stub at 0xf064e098 (aggregate counters + re-emit prologue).
# Re-sign (qtestsign -v3). Produces adsp-m6-signed.mbn. Deploy NON-crashing:
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m6trace.o
"$OC" -O binary --only-section=.text m6trace.o m6trace.bin
echo "cave stub: $(wc -c < m6trace.bin) bytes"

# sanity: the re-emitted prologue (raw .long 0x70604010,0xebf41c10) must be present
python3 - <<'PY'
b=open("m6trace.bin","rb").read()
...
python3 - "$STOCK" m6trace.bin "$OUT_UNS" "$INJECT_VADDR" "$CAVE_VADDR" <<'PY'
