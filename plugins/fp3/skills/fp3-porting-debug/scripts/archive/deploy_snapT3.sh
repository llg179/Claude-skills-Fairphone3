...
# deploy_snapT3.sh — SNT3 COLD-BOOT deploy (boot-time-once event).
...
SIG=$FP3_ROOT/adsp-snapT3-signed.mbn
...
echo "== [2] deploy patched (SNT3) =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapT3-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapT3-signed.mbn $FWDIR/adsp.mbn; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
sleep 12
echo "== [4] read SMEM (SNT3 stash) =="
$SCP smem_snapT3_read.py "$DEV:/tmp/smem_snapT3_read.py" >/dev/null 2>&1
echo "---- SNT3 READOUT ----"
r "python3 /tmp/smem_snapT3_read.py"
echo "----------------------"
