...
# Stage-7 (SNP7): dump the f0191c68 config-struct window at the caller splice
...
a=os.path.join(tmp,"snap7.s");o=os.path.join(tmp,"snap7.o");b=os.path.join(tmp,"snap7.bin")
open(a,"w").write(ASM)
...
open(OUT,"wb").write(data); open(os.path.join(tmp,"cave7.bin"),"wb").write(cave)
