#!/bin/bash
# snapCGP COLD-BOOT deploy (boot-time-once leaf event, fixed-VA stash).
...
SIG=$FP3_ROOT/adsp-snapCGP-signed.mbn
...
$SCP "$SIG" "$DEV:/tmp/adsp-snapCGP-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapCGP-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
$SCP smem_snapCGP_read.py "$DEV:/tmp/smem_snapCGP_read.py" >/dev/null 2>&1
echo "---- CGP READOUT ----"
r "python3 /tmp/smem_snapCGP_read.py"
echo "----------------------"
