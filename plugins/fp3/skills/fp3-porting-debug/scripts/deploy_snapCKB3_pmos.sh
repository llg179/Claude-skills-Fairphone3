...
SIG=$FP3_ROOT/adsp-snapCKB3-signed.mbn
...
$SCP "$SIG" "$DEV:/tmp/adsp-snapCKB3-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapCKB3-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
...
$SCP smem_snapCKB3_read.py "$DEV:/tmp/smem_snapCKB3_read.py" >/dev/null 2>&1
echo "---- CGP READOUT ----"
r "python3 /tmp/smem_snapCKB3_read.py"
echo "----------------------"
