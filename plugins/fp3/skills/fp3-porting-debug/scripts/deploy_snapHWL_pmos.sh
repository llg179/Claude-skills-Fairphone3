...
SIG=$FP3_ROOT/adsp-snapHWL-signed.mbn
...
echo "== [2] deploy patched (HWL0) =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapHWL-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapHWL-signed.mbn $FWDIR/adsp.mbn; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
sleep 12
echo "== [4] read SMEM (HWL0 stash) =="
$SCP smem_snapHWL_read.py "$DEV:/tmp/smem_snapHWL_read.py" >/dev/null 2>&1
echo "---- HWL0 READOUT ----"
r "python3 /tmp/smem_snapHWL_read.py"
echo "----------------------"
