...
OLD_FN=0xf04df0ac
OUT_UNS=adsp-m11.mbn
OUT_SIG=adsp-m11-signed.mbn
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m11trace.o
"$OC" -O binary --only-section=.text m11trace.o m11trace.bin
echo "cave stub: $(wc -c < m11trace.bin) bytes"

python3 - "$STOCK" m11trace.bin "$OUT_UNS" "$CAVE_VADDR" "$VTABLE_VADDR" "$OLD_FN" <<'PY'
