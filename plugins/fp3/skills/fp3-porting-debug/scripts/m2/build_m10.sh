#!/bin/bash
# build_m10.sh — M10 slim CBCR enable trace. TWO edits to stock adsp.mbn:
#   (a) cave stub m10trace.s at 0xf064e098 (replicates f04df0ac enable + ring-logs),
...
OLD_FN=0xf04df0ac
OUT_UNS=adsp-m10.mbn
OUT_SIG=adsp-m10-signed.mbn
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m10trace.o
"$OC" -O binary --only-section=.text m10trace.o m10trace.bin
echo "cave stub: $(wc -c < m10trace.bin) bytes"

python3 - "$STOCK" m10trace.bin "$OUT_UNS" "$CAVE_VADDR" "$VTABLE_VADDR" "$OLD_FN" <<'PY'
