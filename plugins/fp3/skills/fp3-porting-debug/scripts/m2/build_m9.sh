#!/bin/bash
# build_m9.sh — M8 post-DAL-enable rc capture. Splices TWO pieces into stock adsp.mbn:
#   (a) a 4-byte relative `jump 0xf064e098` OVER the single-word packet
#       `{ jump 0xf001a854 }` at 0xf04bfb00 (right after the DAL enable call),
#   (b) the m9trace.s cave stub at 0xf064e098 (rc capture + counters).
# Re-sign (qtestsign -v3). Produces adsp-m9-signed.mbn. NON-crashing; the cave
...
INJECT_VADDR=0xf0617df4        # immext word a940fa0f, first word of f0617928 exit packet
OUT_UNS=adsp-m9.mbn
OUT_SIG=adsp-m9-signed.mbn
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m9trace.o
"$OC" -O binary --only-section=.text m9trace.o m9trace.bin
echo "cave stub: $(wc -c < m9trace.bin) bytes"

# sanity: the final trailer jump target 0xf001a854 must be assembled somewhere
python3 - <<'PY'
b=open("m9trace.bin","rb").read()
# 0xf001a854 appears as a ## immediate (little-endian embedded); just assert nonzero size
...
python3 - "$STOCK" m9trace.bin "$OUT_UNS" "$INJECT_VADDR" "$CAVE_VADDR" <<'PY'
...
assert orig==bytes.fromhex("a940fa0f"), "inject site is not the { jump 0xf001a854 } word! got %s"%orig.hex()
buf[io:io+4]=jb
open(out,"wb").write(buf)
print("cave  @ file 0x%x : %d bytes"%(co,len(code)))
print("inject@ file 0x%x : %s -> %s (replaces { jump 0xf001a854 })"%(io, orig.hex(), jb.hex()))
PY
