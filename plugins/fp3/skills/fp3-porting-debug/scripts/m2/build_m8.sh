#!/bin/bash
# build_m8.sh — M8 post-DAL-enable rc capture. Splices TWO pieces into stock adsp.mbn:
...
#   (b) the m8trace.s cave stub at 0xf064e098 (rc capture + counters).
# Re-sign (qtestsign -v3). Produces adsp-m8-signed.mbn. NON-crashing; the cave
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m8trace.o
"$OC" -O binary --only-section=.text m8trace.o m8trace.bin
echo "cave stub: $(wc -c < m8trace.bin) bytes"

# sanity: the final trailer jump target 0xf04bfb5c must be assembled somewhere
python3 - <<'PY'
b=open("m8trace.bin","rb").read()
...
python3 - "$STOCK" m8trace.bin "$OUT_UNS" "$INJECT_VADDR" "$CAVE_VADDR" <<'PY'
