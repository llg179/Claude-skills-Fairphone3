...
# Re-sign (qtestsign -v3). Produces adsp-m13-signed.mbn.
set -euo pipefail
cd "$(dirname "$0")"
...
INJECT_VADDR=0xf04c97d0        # first word of the gate entry packet; orig = 02 4b 10 91
ORIG_HEX=024b1091
OUT_UNS=adsp-m13.mbn
OUT_SIG=adsp-m13-signed.mbn
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m13trace.o
"$OC" -O binary --only-section=.text m13trace.o m13trace.bin
echo "cave stub: $(wc -c < m13trace.bin) bytes"

python3 - "$STOCK" m13trace.bin "$OUT_UNS" "$INJECT_VADDR" "$CAVE_VADDR" "$ORIG_HEX" <<'PY'
