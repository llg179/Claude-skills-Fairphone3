#!/bin/bash
# build_m14.sh — M14 framer-GATE 5-condition trace. Splices TWO pieces into stock:
#   (a) the m14trace.s cave stub at 0xf064e098 (replicates the f04c97d0 gate +
...
# Re-sign (qtestsign -v3). Produces adsp-m14-signed.mbn.
set -euo pipefail
cd "$(dirname "$0")"
...
INJECT_VADDR=0xf04bf6ec        # first word of the gate entry packet; orig = 42 c0 80 91 = { r2 = memw(r0+#8) }
ORIG_HEX=42c08091
OUT_UNS=adsp-m14.mbn
OUT_SIG=adsp-m14-signed.mbn
...
"$MC" --arch=hexagon --mcpu=hexagonv60 --filetype=obj "$STUB" -o m14trace.o
"$OC" -O binary --only-section=.text m14trace.o m14trace.bin
echo "cave stub: $(wc -c < m14trace.bin) bytes"

python3 - "$STOCK" m14trace.bin "$OUT_UNS" "$INJECT_VADDR" "$CAVE_VADDR" "$ORIG_HEX" <<'PY'
