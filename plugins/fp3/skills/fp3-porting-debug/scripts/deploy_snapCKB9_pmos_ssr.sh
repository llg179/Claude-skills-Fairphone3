...
SIG=$FP3_ROOT/adsp-snapCKB9-signed.mbn
...
echo "== [3] deploy patched CKB9 =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapCKB9-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapCKB9-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
...
echo "== [5] read SMEM (CKB9 stash) =="
$SCP smem_snapCKB9_read.py "$DEV:/tmp/smem_snapCKB9_read.py" >/dev/null 2>&1
echo "---- CKB9 READOUT (pmOS/dead) ----"
r "python3 /tmp/smem_snapCKB9_read.py"
echo "----------------------------------"
