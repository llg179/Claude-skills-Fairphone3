...
# deploy_snapCGP_ut.sh — v4 FIXED-VA positive control (does f019abb0 run at UT boot?).
# UT is PIL: firmware = split on vfat /dev/mmcblk0p1 (RO in LXC). Recipe (proven, T3-A):
#   push whole CGP-injected p1 image -> dd over /dev/mmcblk0p1 -> clean reboot -> read SMEM ring.
...
set -uo pipefail
IMG=$FP3_ROOT/ut-p1-ckb2.img
RDR=$(dirname "$0")/smem_snapCKB2_read.py
PW="$FP3_PW"
...
echo "== [2] push CGP p1 image ($(du -h $IMG|cut -f1)) =="
timeout 180 adb push "$IMG" /home/phablet/ut-p1-ckb2.img 2>&1 | tail -1
adb push "$RDR" /home/phablet/smem_snapCKB2_read.py 2>&1 | tail -1
echo "== [3] dd over /dev/mmcblk0p1 =="
A "dd if=/home/phablet/ut-p1-ckb2.img of=/dev/mmcblk0p1 bs=1M conv=fsync 2>&1 | tail -1; sync; md5sum /firmware/image/adsp.mdt" 120
echo "== [4] clean reboot =="
...
echo "== [6] READ CGP ring (SMEM) =="
echo "---- CKB2 GOLDEN READOUT ----"
A "python3 /home/phablet/smem_snapCKB2_read.py" 40
echo "----------------------------"
echo "DONE -> OK (CGP cave left in place; framer alive; non-crashing)"
