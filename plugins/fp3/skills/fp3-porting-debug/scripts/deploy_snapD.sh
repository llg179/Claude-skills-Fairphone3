...
# deploy_snapD.sh — SNPD COLD-BOOT deploy (boot-time-once event).
...
SIG=$FP3_ROOT/adsp-snapD-signed.mbn
...
echo "== [2] deploy patched (SNPD) =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapD-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapD-signed.mbn $FWDIR/adsp.mbn; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
sleep 12
echo "== [4] read SMEM (SNPD stash) =="
$SCP smem_snapD_read.py "$DEV:/tmp/smem_snapD_read.py" >/dev/null 2>&1
echo "---- SNPD READOUT ----"
r "python3 /tmp/smem_snapD_read.py"
echo "----------------------"
