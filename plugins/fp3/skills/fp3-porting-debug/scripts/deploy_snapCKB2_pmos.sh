...
SIG=$FP3_ROOT/adsp-snapCKB2-signed.mbn
...
$SCP "$SIG" "$DEV:/tmp/adsp-snapCKB2-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapCKB2-signed.mbn $FWDIR/adsp.mbn; sync; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
$SCP smem_snapCKB2_read.py "$DEV:/tmp/smem_snapCKB2_read.py" >/dev/null 2>&1
echo "---- CGP READOUT ----"
r "python3 /tmp/smem_snapCKB2_read.py"
echo "----------------------"
