...
STAGE=$HOME/frs7-staging
RES=/root/frs7-result.txt
RP=remoteproc2
MIN_FREE_MB=200          # abort if less than this free (folyt.134: 210M looped, 272M stable)

echo "=== $(date) FRS7 onboard run (DEAD side) ===" > "$RES"; sync
{
...
PY
echo "-- deploy FRS7 --"; cp $STAGE/adsp-snapFRS7-signed.mbn $FW/adsp.mbn; sync; md5sum $FW/adsp.mbn
...
echo "-- FRS7 ctx-scan readout (DEAD side) --" >> "$RES"
python3 $STAGE/smem_snapFRS7_read.py >> "$RES" 2>&1; sync
