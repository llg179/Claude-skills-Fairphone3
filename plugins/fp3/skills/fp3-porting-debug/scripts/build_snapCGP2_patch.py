...
# So the actual driver fn = memw(memw(handle+0x3c)+0). Capture it (+ neighbours) to disasm
# the real CBCR poke offline (if it lands in text). All derefs are ones 0xf01a1280 itself
# does -> safe. Same proven splice/exit as snapCGP (f04bfba0 -> f04bfbd8). Magic 'CGP2'.
#   +0x00 'CGP2' +0x04 handle +0x08 handle+0x38 +0x0c s3c=memw(handle+0x3c)
#   +0x10 memw(s3c+0)=DRIVER FN +0x14 memw(s3c+4) +0x18 handle+0x40 +0x1c rc
import os
...
OUT=f"{FP3_ROOT}/adsp-snapCGP2.mbn"
...
ASM=r"""
    ...
    { r4 = memw(r3+#0x38) }
    { memw(r1+#0x08) = r4 }
    { r5 = memw(r3+#0x3c) }            // s3c = subobj ptr
    { memw(r1+#0x0c) = r5 }
    { r4 = memw(r3+#0x40) }
    { memw(r1+#0x18) = r4 }
    { p0 = cmp.eq(r5,#0x0); if (p0.new) jump:nt .Lrc }
    { r4 = memw(r5+#0x0) }             // ★ DRIVER FN = memw(memw(handle+0x3c)+0)
    { memw(r1+#0x10) = r4 }
    { r4 = memw(r5+#0x4) }
    { memw(r1+#0x14) = r4 }
.Lrc:
...
a=os.path.join(here,"snapCGP2.s");o=os.path.join(here,"snapCGP2.o");b=os.path.join(here,"snapCGP2.bin")
open(a,"w").write(ASM)
