...
# So static address-grep can't find the clock/pad sibling blocks. Instead: at the both-sides mode-update
...
#   +0x00 'FRS7' | +0x04 count | +0x08.. up to 8 pairs of (offset,value) [8 bytes each].
import os
...
OUT=f"{FP3_ROOT}/adsp-snapFRS7.mbn"
DELTA=0xf00fd000
...
.Lloop:
    ...
    {{ p0 = cmp.eq(r4,##0xee000000); if (!p0.new) jump:nt .Lnext }}   // not an LPASS pointer -> skip
...
a=os.path.join(here,"snapFRS7.s");o=os.path.join(here,"snapFRS7.o");b=os.path.join(here,"snapFRS7.bin")
open(a,"w").write(ASM)
