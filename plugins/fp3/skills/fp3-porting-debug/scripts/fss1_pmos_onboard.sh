#!/bin/bash
# Runs ON pmOS (slot_b). Deploys FSS1 via SSR-reload (NO reboot), captures the framer STATUS registers
...
STAGE=$HOME/fss1-staging
RES=/root/fss1-result.txt
RP=remoteproc2
MIN_FREE_MB=200

echo "=== $(date) FSS1 onboard run (DEAD side, privileged framer-status snapshot) ===" > "$RES"; sync
{
...
PY
echo "-- deploy FSS1 --"; cp $STAGE/adsp-snapFSS1-signed.mbn $FW/adsp.mbn; sync; md5sum $FW/adsp.mbn
...
echo "-- FSS1 privileged framer-status trace (DEAD side) --" >> "$RES"
python3 $STAGE/smem_snapFSS1_read.py >> "$RES" 2>&1; sync
