...
# deploy_snapT3.sh — HVA0 COLD-BOOT deploy (boot-time-once event).
...
SIG=$FP3_ROOT/adsp-snapVA-signed.mbn
...
echo "== [2] deploy patched (HVA0) =="
$SCP "$SIG" "$DEV:/tmp/adsp-snapVA-signed.mbn" >/dev/null 2>&1
r "cp /tmp/adsp-snapVA-signed.mbn $FWDIR/adsp.mbn; echo -n patched=; md5sum $FWDIR/adsp.mbn"
echo "== [3] COLD reboot =="
do_reboot
...
sleep 12
echo "== [4] read SMEM (HVA0 stash) =="
$SCP smem_snapVA_read.py "$DEV:/tmp/smem_snapVA_read.py" >/dev/null 2>&1
echo "---- HVA0 READOUT ----"
r "python3 /tmp/smem_snapVA_read.py"
echo "----------------------"
