...
set -uo pipefail
IMG=$FP3_ROOT/ut-p1-cgp2.img
RDR=$(dirname "$0")/smem_snapCGP2_read.py
PW="$FP3_PW"
...
timeout 180 adb push "$IMG" /home/phablet/ut-p1-cgp2.img 2>&1 | tail -1
adb push "$RDR" /home/phablet/smem_snapCGP2_read.py 2>&1 | tail -1
echo "== [3] dd over /dev/mmcblk0p1 =="
A "dd if=/home/phablet/ut-p1-cgp2.img of=/dev/mmcblk0p1 bs=1M conv=fsync 2>&1 | tail -1; sync; md5sum /firmware/image/adsp.mdt" 120
echo "== [4] clean reboot =="
...
echo "---- CGP2 GOLDEN READOUT ----"
A "python3 /home/phablet/smem_snapCGP2_read.py" 40
echo "----------------------------"
