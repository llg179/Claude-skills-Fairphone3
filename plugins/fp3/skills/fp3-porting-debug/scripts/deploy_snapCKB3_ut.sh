...
set -uo pipefail
IMG=$FP3_ROOT/ut-p1-ckb3.img
RDR=$(dirname "$0")/smem_snapCKB3_read.py
PW="$FP3_PW"
...
timeout 180 adb push "$IMG" /home/phablet/ut-p1-ckb3.img 2>&1 | tail -1
adb push "$RDR" /home/phablet/smem_snapCKB3_read.py 2>&1 | tail -1
echo "== [3] dd over /dev/mmcblk0p1 =="
A "dd if=/home/phablet/ut-p1-ckb3.img of=/dev/mmcblk0p1 bs=1M conv=fsync 2>&1 | tail -1; sync; md5sum /firmware/image/adsp.mdt" 120
echo "== [4] clean reboot =="
...
A "python3 /home/phablet/smem_snapCKB3_read.py" 40
echo "----------------------------"
