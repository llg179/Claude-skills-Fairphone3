...
set -uo pipefail
IMG=$FP3_ROOT/ut-p1-ckb7b.img
RDR=$(dirname "$0")/smem_snapCKB7b_read.py
PW="$FP3_PW"
...
echo "== [2] push CKB7b p1 image ($(du -h $IMG|cut -f1)) =="
timeout 240 adb push "$IMG" /home/phablet/ut-p1-ckb7b.img 2>&1 | tail -1
adb push "$RDR" /home/phablet/smem_snapCKB7b_read.py 2>&1 | tail -1
echo "== [3] dd over /dev/mmcblk0p1 =="
A "dd if=/home/phablet/ut-p1-ckb7b.img of=/dev/mmcblk0p1 bs=1M conv=fsync 2>&1 | tail -1; sync; md5sum /firmware/image/adsp.mdt" 180
echo "== [4] clean reboot =="
...
echo "== [6] READ CKB7b (SMEM) — golden branch-enable =="
echo "---- CKB7b GOLDEN READOUT (UT) ----"
A "python3 /home/phablet/smem_snapCKB7b_read.py" 40
echo "----------------------------------"
echo "DONE -> OK"
