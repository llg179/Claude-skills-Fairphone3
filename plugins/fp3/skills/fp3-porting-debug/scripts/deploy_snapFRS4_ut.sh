...
# deploy_snapFRS4_ut.sh — GOLDEN FRS4 on the UT oracle (framer ALIVE, PIL cold boot).
# Reads back the framer-branch CBCR (CLK_OFF=bit31) + root guess after the enable, to diff vs the
# pmOS/dead side (folyt.128). UT is PIL: push FRS4-injected p1 -> dd over /dev/mmcblk0p1 -> clean
...
set -uo pipefail
IMG=$FP3_ROOT/ut-p1-frs4.img
RDR=$(dirname "$0")/smem_snapFRS4_read.py
PW="$FP3_PW"
...
echo "== [2] push FRS4 p1 image ($(du -h $IMG|cut -f1)) =="
timeout 240 adb push "$IMG" /home/phablet/ut-p1-frs4.img 2>&1 | tail -1
adb push "$RDR" /home/phablet/smem_snapFRS4_read.py 2>&1 | tail -1
echo "== [3] dd over /dev/mmcblk0p1 =="
A "dd if=/home/phablet/ut-p1-frs4.img of=/dev/mmcblk0p1 bs=1M conv=fsync 2>&1 | tail -1; sync; md5sum /firmware/image/adsp.mdt" 180
...
echo "== [6] READ FRS4 (SMEM) — GOLDEN framer-regs =="
echo "---- FRS4 GOLDEN READOUT (UT/alive) ----"
A "python3 /home/phablet/smem_snapFRS4_read.py" 40
echo "----------------------------------------"
