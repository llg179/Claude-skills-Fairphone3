...
- **Kernel-tree commits are LOCAL** (origin=upstream), NEVER push. *(folyt.208 nuance: `origin` is still forbidden, but the user's own fork `github.com/llg179/linux` branch `fp3-709` IS a valid push target — see the folyt.208 correction in §"Artefacts".)*
